...
def api_copy_application_template(req: CopyApplicationTemplateRequest):
    """
    Safe copy 40.Application folder structure from N drive template to local 40.Application.

    This endpoint intentionally copies folders only. It does not copy files and does not overwrite anything.
    """
    try:
        source = Path(SOURCE_CALIBRATION_TEMPLATE)
        destination = _resolve_application_destination(req.destination_application_dir, req.destination_path)
        result = _safe_copy_folder_tree(source, destination)
        return {
            "success": True,
            "status": "success",
            "message": "40.Application folder structure initialized by safe copy.",
            **result,
        }
...
# 兼容旧前端：保留 /copy-folder，但内部改为 safe 复制文件夹结构。
...
async def copy_folder_to_destination(request_data: CopyRequest):
    try:
        ...
        result = _safe_copy_folder_tree(Path(SOURCE_CALIBRATION_TEMPLATE), destination)
        return {
            "success": True,
            "status": "success",
            "message": "40.Application folder structure initialized by safe copy.",
