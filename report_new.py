...
from services.word.toc import mark_fields_for_update_on_open_batch
    ...
    *,
    ...
    try:
        for index, template_path in enumerate(template_paths, start=1):
            ...
            logger.info(
                "[TCD08] (%s/%s) Queued document for update-on-open field marking.",
                index,
                len(template_paths),
            )
        ...
        if toc_update_paths:
            logger.info(
                "[TCD08] Marking fields for update on open in %s document(s).",
                len(toc_update_paths),
            )
            step_start = time.perf_counter()
            try:
                update_summary = mark_fields_for_update_on_open_batch(toc_update_paths)
                logger.info(
                    "[TCD08] Field update-on-open marking took %.2fs. changed=%s total=%s",
                    time.perf_counter() - step_start,
                    update_summary["changed"],
                ...
                logger.warning(
                    "[TCD08] Failed to mark fields for update on open, but report generation continues. error=%s",
                    exc,
                    exc_info=True,
                )
