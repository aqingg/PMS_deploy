...
def mark_fields_for_update_on_open(package_path: Path) -> bool:
    """Mark a Word package so Microsoft Word updates fields when opened."""
    package_path = Path(package_path)
    if not package_path.is_file():
        raise FileNotFoundError(f"Word document not found: {package_path}")
    ...
    with zipfile.ZipFile(package_path, "r") as archive:
        try:
            ...
            raise RuntimeError(
                f"Word package does not contain word/settings.xml: {package_path}"
            ) from exc

    word_ns = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    update_fields_tag = f"{{{word_ns}}}updateFields"
    value_attr = f"{{{word_ns}}}val"
    root = ET.fromstring(settings_xml)
    update_fields_nodes = root.findall(update_fields_tag)

    changed = False
    if update_fields_nodes:
        primary_node = update_fields_nodes[0]
        if primary_node.get(value_attr) != "true":
            primary_node.set(value_attr, "true")
            changed = True
        for duplicate_node in update_fields_nodes[1:]:
            root.remove(duplicate_node)
            changed = True
    else:
        update_fields_node = ET.SubElement(root, update_fields_tag)
        update_fields_node.set(value_attr, "true")
        changed = True

    if not changed:
        return False

    updated_settings_xml = ET.tostring(
        root,
        encoding="utf-8",
    ...
    write_zip_with_replacement(
        package_path,
        package_path,
        {"word/settings.xml": updated_settings_xml},
    )
    return True
