...
from services.word.package import (
    MacroSupportMembers,
    mark_fields_for_update_on_open as mark_package_fields_for_update_on_open,
...
def mark_fields_for_update_on_open(document_path: Path) -> bool:
    """Mark one Word document so fields refresh when opened in Word."""
    return mark_package_fields_for_update_on_open(Path(document_path))


def mark_fields_for_update_on_open_batch(
    document_paths: Iterable[Path],
) -> dict[str, int]:
    """Mark multiple Word documents without starting Microsoft Word."""
    total = 0
    changed = 0
    for document_path in document_paths:
        total += 1
        if mark_fields_for_update_on_open(Path(document_path)):
            changed += 1
